...
class SavedPlayerCNN(Player):
    def __init__(self, env, name='SavedPlayerCNN', model_prefix=None):
        super(SavedPlayerCNN, self).__init__(env, name)
...
class ConnectFourEnv(gym.Env):
    ...
    def __init__(self, board_shape=(6, 7), window_width=512, window_height=512):
        ...
        self.__board = np.random.randint(-1, 1, self.board_shape)
    ...
    def run(self, player1: Player, player2: Player, render=False) -> ResultType:
        ...
        while not done:
            ...
            player.learn(state=state_hist[-3] * cp(), action=act_hist[-2], state_next=state_hist[-1] * cp(),
    ...
    def reset(self, board: Optional[np.ndarray] = None) -> np.ndarray:
        ...
        if board is None:
            self.__board = np.zeros(self.board_shape, dtype=int)
            self.__board[(np.random.randint(1, 6, 1), np.random.randint(1, 7, 1))] = np.random.randint(-1, 2, 1)
            self.__board[(np.random.randint(1, 6, 1), np.random.randint(1, 7, 1))] = np.random.randint(-1, 2, 1)
        else:
            self.__board = board
        self.__rendered_board = self._update_board_render()
